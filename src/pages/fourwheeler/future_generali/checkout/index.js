...
import {
  actionGetFGPincodeMaster,
  actionGetProposal,
  actionSubmitProposal,
  actionUpdateProposal,
  resetPincodeDetails,
  resetProposal,
  actionKYCSubmit,
  actionKYCStatus,
...
import KycRedirect from "./auto_redirectform";
...
export default function CheckoutStepper() {
  ...
  const { getFGProposalDetails, getPincodeDetails, kycSubmit, kycStatus } =
  ...
  const submitKYCDetails = (data) => {
    ...
    //   gender_id: data.gender_master_id.value,
    //   kyc_type_id: data.ckyc_doc_type_master_id.value,
    ...
    if (proposalObjectDetails?.pjCarQuotationDetails?.proposer_type !== 'Individual') {
      let params = {
        ...
        reference_id: QUOTE_ID,
        kyc_reference_number: data.kyc_reference_number,
        date_of_incorporation: dayjs(data.date_of_incorporation).format("YYYY-MM-DD"),
        kyc_type_id: data.ckyc_doc_type_master_id.value,
        first_name: data.first_name,
      ...
      console.log(params);
      setLoading(true);
      dispatch(actionKYCSubmit(params));
    }
    else {
      let params = {
        kyc_reference_number: data.kyc_reference_number,
  ...
  useEffect(() => {
    if (
      ...
      if (matchedGender) {
        kycDetailsSetValue("gender_master_id", matchedGender);
      }
    }
    ...
    if (getFGProposalDetails !== null) {
      ...
      if (getFGProposalDetails?.result == true) {
        ...
        kycDetailsSetValue(
          ...
          getFGProposalDetails?.response?.kyc_json?.kyc_reference_number || ""
        );
        ...
        let kycDocTypes = transformKYCDocTypes(
          getFGProposalDetails?.response?.masters?.kyc_master
        );
            ...
            <div style={{ textAlign: "center", marginTop: "50px" }}>
              ...
              <form ref={formRef} id="KYCFailedFormTag" method="post" name="redirect" action="https://ekyc-uat.fggeneral.in/kyc-verification?access=25a8c3ac0a90ca08b41fcf6d5aafc5876aaa6d3dd0c84a2ee698c3394af634a474444f011149a61ed31964fa985f">
                ...
                <input
                  ...
                  // value={`http://localhost:3000/fourwheeler/future_generali/proposal_checkout/?quoteId=${QUOTE_ID}&proposalId=${PROPOSAL_ID}`}

                value={`https://car-insurance.kaitotech.com/fourwheeler/future_generali/proposal_checkout/?quoteId=${QUOTE_ID}&proposalId=${PROPOSAL_ID}`}
                />
                            ...
                            {/* <Controller
                              ...
                              render={({ field: { value, onChange } }) => (
                                <CustomTextField
                                  ...
                                  helperText={kycDetailsErrors?.first_name ? kycDetailsErrors.first_name.message : ""}
                                />
                              )}
                            /> */}
                            <Controller
                              ...
                              render={({ field: { value, onChange } }) => (
                                <CustomTextField
                                  ...
                                  placeholder='Enter Company Name'
                                  aria-describedby='validation-basic-company-name'
                                  error={Boolean(kycDetailsErrors?.first_name)}
                                  helperText={kycDetailsErrors?.first_name?.message || ""}
                                />
                              )}
                            />
                          </Grid>
                        ...
                        {/* <Grid item xs={12} sm={6}>
                          <Controller
                            ...
                            rules={{
                              required: "Reference number is required",
                              pattern: {
                                value: /^[A-Z0-9]{5,25}$/, // 5–25 uppercase letters and numbers
                                message:
                                  "Reference number must be 5–25 uppercase letters and numbers (no spaces or special characters)",
                              },
                            }}
                            render={({ field: { value, onChange } }) => (
                              <CustomTextField
